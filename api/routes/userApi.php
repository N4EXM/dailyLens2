...
try {
    ...
    switch ($action) {
        ...
        case 'register':
            if (empty($input['username']) || empty($input['password']) || empty($input["firstName"]) || empty($input["lastName"]) || empty($input["email"])) {
                throw new InvalidArgumentException('All fields are required');
            }
            $response = register($pdo, $input['username'], $input['password'], $input['email'],$input['firstName'],$input['lastName']);
            break;
